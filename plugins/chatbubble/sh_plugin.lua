...
if CLIENT then
    ...
    hook.Add("PostDrawTranslucentRenderables", "ixChatBubbles", function()
        for client, data in pairs(bubbles) do
            ...
            if textWidth > maxWidth then -- Truncate text if too wide
            ...
            draw.SimpleTextOutlined(text, "CloseCaption_Normal", 0, 0, Color(255, 255, 150, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, alpha))
            cam.End3D2D()
        end
    end)
    ...
    net.Receive("ixChatBubble", function()
        ...
        if IsValid(client) then
            ...
            if existing and existing.baseText == text then -- If same message is sent again, increment the counter
...
if SERVER then
    ...
    function PLUGIN:PlayerMessageSend(speaker, chatType, text)
        ...
        print("[ChatBubbles] PlayerMessageSend called:", speaker, chatType, text)
        local allowedTypes = {
            ic = true,
            w = true,
            y = true,
        ...
        if allowedTypes[chatType] then
            ...
            print("[ChatBubbles] Sending bubble:", displayText)
            net.Start("ixChatBubble")
            net.WriteEntity(speaker)
            net.WriteString(displayText)
            net.Broadcast()
        else
            print("[ChatBubbles] Chat type not allowed:", chatType)
        end
    end
end