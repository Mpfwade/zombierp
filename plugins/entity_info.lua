    ...
    function PLUGIN:HUDPaintBackground()
        ...
        for k, v in pairs(drawInfo) do
            ...
            if IsValid(entity) then
                ...
                if entity.UIName then
                    ix.util.DrawText(entity.UIName or "", x, y - (genericHeight / 2), ColorAlpha(ix.config.Get("color"), v.alpha * 255), 1, 1, "Font-Elements20")
                    local descriptionText = entity.UIDesc
                    if descriptionText ~= "" and entity.UIDesc then ix.util.DrawText(descriptionText or "", x, y + descHeight - (genericHeight / 2), ColorAlpha(color_white, v.alpha * 255), 1, 1, "Font-Elements17") end
                    continue
                end
                ...
                if entity:IsPlayer() then
                    ...
                    if character then
                        ...
                        y = y + descHeight
                        local description = character:GetDescription() -- 👇 Add character description display
                        if description and description ~= "" then
                            local wrapped = ix.util.WrapText(description, 300, "ixItemDescFont")
                            for i, line in ipairs(wrapped) do
                                ix.util.DrawText(line, x, y + ((i - 1) * descHeight), ColorAlpha(color_white, v.alpha * 255), 1, 1, "ixItemDescFont")
                            end

                            y = y + (#wrapped * descHeight)
                        end

                        if entity:IsRestricted() then -- Tied/Untying info
                            ix.util.DrawText("They have been tied up.", x, y, ColorAlpha(Color(230, 180, 0), v.alpha * 255), 1, 1, "ixItemDescFont")
                            y = y + descHeight
                        elseif entity:GetNetVar("tying") then
                            ix.util.DrawText("They are being tied up.", x, y, ColorAlpha(Color(230, 180, 0), v.alpha * 255), 1, 1, "ixItemDescFont")
                            y = y + descHeight
                        elseif entity:GetNetVar("untying") then
                            ix.util.DrawText("They are being untied.", x, y, ColorAlpha(Color(230, 180, 0), v.alpha * 255), 1, 1, "ixItemDescFont")
                            y = y + descHeight
                        end

                        local injureText, injureTextColor = hook.Run("GetInjuredText", entity) -- Injured info
                        if injureText then
                            ix.util.DrawText(L(injureText) or "", x, y, ColorAlpha(injureTextColor, v.alpha * 255), 1, 1, "ixItemDescFont")
                            y = y + descHeight
                        end
